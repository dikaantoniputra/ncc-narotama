...
@section('content')
    ...
    <section class="bg-[#F7F7F7] lg:pl-[80px] lg:pr-[80px] lg:pt-[60px] max-sm:p-[40px]">
        <div>
            ...
            <form action="{{ route('user.page.course') }}" method="GET">
                <div class="mt-[60px] bg-white grid lg:grid-cols-3 grid-cols-2 lg:w-[943px] h-[80px] mx-auto items-center rounded-[18px] justify-between p-[12px] gap-[24px]">
                    <div class="bg-[#F7F7F7] flex items-center px-[24px] rounded-[6px]">
                        <input class="w-full border-none focus:border-none focus:outline-none focus:ring-0 bg-[#F7F7F7]" name="nama_pelatihan" placeholder="Cari Pelatihan" type="text">
        ...
        <section class="bg-[#4176CF] lg:px-[80px] lg:py-[80px] py-[40px] px-[40px]">
            ...
            <div class="mt-[30px]">
                ...
                <div class="text-white mt-[30px]">
                    <div>Halaman : {{ $searchCourse->currentPage() }}</div>
                    <div>Jumlah Halaman : {{ $searchCourse->total() }}</div>
                    <div>Data Per Halaman : {{ $searchCourse->perPage() }}</div>
                </div>
            </div>
        </section>
    </section>
                    ...
                    <tbody class="bg-white">
                        @forelse ($registeredCourse as $item)
                            <tr class="text-center text-[#606060]">
                                ...
                                <td class="px-[24px] py-[16px]">
                                    {{ \Carbon\Carbon::parse($item->pelatihan->created_at)->format('d M Y') }}
                                </td class="px-[24px] py-[16px]">
                                <td class="px-[24px] py-[16px]">
                                ...
                                <td class="px-[24px] py-[16px]">
                                    <a href="{{ route('user.page.detailCourse', $item->pelatihan_id) }}" class="px-[20px] py-[10px] bg-[#4176CF] rounded-[6px] text-white">Lihat Kegiatan</a>
                                </td>
                            </tr>
                        @empty
                            Anda belum mendaftar pelatihan.
                        @endforelse
                    </tbody>
                </table>
            </div>
